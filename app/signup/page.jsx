...
import { Mail, Lock, ArrowRight, User, Github, Twitter, Sparkles } from 'lucide-react';
...
export default function SignupPage() {
    ...
    return (
        <div className="min-h-screen grid lg:grid-cols-2 bg-[#6366F1] relative overflow-hidden">
            {/* Background Animations */}
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                <motion.div
                    animate={{ rotate: -360 }}
                    transition={{ duration: 60, repeat: Infinity, ease: "linear" }}
                    className="absolute -bottom-1/2 -left-1/2 w-[100vw] h-[100vw] rounded-full border-2 border-white/10 border-dashed"
                />
            </div>
            ...
            <div className="flex items-center justify-center p-8 relative z-10">
                ...
                >
                    <div className="text-center mb-8">
                        ...
                        <h2 className="text-xl text-white/90 font-medium">Join the Adventure!</h2>
                    </div>

                    <form onSubmit={handleSubmit} className="space-y-5">
                        {/* Fields omitted mostly, keeping structure */}
                        <div className="space-y-2">
                            <label className="text-sm font-medium text-white/80 ml-1">Full Name</label>
                            <div className="relative group">
                                <User className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-white/50 group-focus-within:text-white transition-colors" />
                                <input
                                    type="text"
                                    value={name}
                        ...
                        <motion.button
                            ...
                            whileTap={{ scale: 0.98 }}
                            type="submit"
                            className="w-full bg-white text-[#6366F1] font-bold py-3 rounded-xl shadow-lg hover:bg-gray-100 transition-colors flex items-center justify-center gap-2 mt-4"
                        >
                            <Sparkles className="h-5 w-5" /> Start Journey
                        </motion.button>
                    </form>
                    ...
                    <p className="mt-8 text-center text-sm text-white/60">
                        ...
                        <Link href="/login" className="font-medium text-white hover:underline decoration-2 underline-offset-4">
                            Log in
                        </Link>
                    </p>
                </motion.div>
            ...
            <div className="hidden lg:flex items-center justify-center bg-white/5 backdrop-blur-sm relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-bl from-purple-400/20 to-transparent" />
                <HeroVisual />
            </div>
        </div>
    );
